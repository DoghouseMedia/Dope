define("dope/rpc/Rest", ["dojo", "dojox"], function(dojo, dojox) {
...
//	  	| services.myRestService['delete']("parameters");
		...
		drr = dope.rpc.Rest = function(/*String*/path, /*Boolean?*/isJson, /*Object?*/schema, /*Function?*/getRequest){
			// summary:
			...
			service._getRequest = getRequest || function(id, args){
				var url = new dope.utils.Url(path);
				if(dojo.isObject(id) && id.id){
					url.setAction(id.id);
				}
				if(args && args.sort && !args.queryStr){
					...
					for(var i = 0; i<args.sort.length; i++) {
						...
						sortParts.push(
							sort.attribute + (sort.descending ? ' DESC' : ' ASC')
						);
					}
		...
		drr._get= function(service,id, args){
			...
			var deferred = index(dojo.xhrGet(service._getRequest(id, args)), service, (args.start >= 0 || args.count >= 0), id);
