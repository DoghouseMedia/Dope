...
class Adapter implements \Zend_Auth_Adapter_Interface
{
    ...
    protected function _authenticateSetup()
    {
        ...
        if (null !== $exceptionMessage) {
            throw new \Zend_Auth_Adapter_Exception($exceptionMessage);
        }
    ...
    protected function _authenticateCreateSelect()
    {
        ...
        if (empty($this->_credentialTreatment)) {
            $this->_credentialTreatment = ':credential';
        } elseif (strpos($this->_credentialTreatment, "?") !== false) {
        	$this->_credentialTreatment = str_replace('?', ':credential', $this->_credentialTreatment);
        }
  
        $credentialExpression = '(CASE WHEN u.' . $this->_credentialColumn
            . ' = ' . $this->_credentialTreatment
            . ' THEN 1 ELSE 0 END) AS badwolf_auth_credential_match';

        $sql = "SELECT u, {$credentialExpression} FROM {$this->_tableClass} u WHERE u.{$this->_identityColumn} = :identity";
        
        $query = $this->_em->createQuery($sql);
        $query->setParameter('credential', $this->_credential);
        $query->setParameter('identity', $this->_identity);

        return $query;
    }

    protected function _authenticateQueryDql(\Doctrine\ORM\Query $dqlQueryObject)
    {
        try {
